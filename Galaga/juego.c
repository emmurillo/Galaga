...
#define VELOCIDAD_ATAQUE 0.02
#define VELOCIDAD_MOVIMIENTO 0.2
#define VELOCIDAD_BALA 0.005
...
#define COLOR_TEXTO al_map_rgb(153,255,153)
#define TAM_FUENTE 15
...
ALLEGRO_THREAD * hilo_score;
ALLEGRO_THREAD * hilo_tractor;
ALLEGRO_MUTEX *mutex;
...
void *update_score(ALLEGRO_THREAD *thr, void *datos){
    int total = 0;
    while(JuegoDatos->jugando){
        ...
        //         BAJO BAJO
            ...
            Puntaje = (CANT_MARCIANOS-total)*100;
            RespPuntaje = (Puntaje) + (Nivel-1)*4000;
            total = 0;
            }
...
void *espera_tractor(ALLEGRO_THREAD *thr, void *datos){
    DatosGlobales * mis_datos = malloc(sizeof(DatosGlobales));
    mis_datos = (DatosGlobales*)datos;
    while(JuegoDatos->jugando){
        if(abs(tractor->xTractor+50 - Nave->xNave) < 40){
            al_rest(1);
            matar_nave();
            Nave->colisionado = 1;
        }
...
void *ataque_marcianos(ALLEGRO_THREAD *thr, void *datos){
    ...
    switch(rand){
            ...
            if(BossArray[rand].visible){
                        ...
                        while(((BossArray[rand].yBoss < FILA_NAVE - 100))){
                            al_rest(VELOCIDAD_ATAQUE);
                            BossArray[rand].yBoss += MOV_ATAQUE;
                            ...
                            tractor->xTractor = BossArray[rand].xBoss - 40;
                            tractor->yTractor = BossArray[rand].yBoss + 35;
                            if(abs(tractor->xTractor+50 - Nave->xNave) < 40){
                                matar_nave();
                                Nave->colisionado = 1;
                                al_rest(2);
                                tractor->xTractor = MAS_ALLA;
                                tractor->yTractor = MAS_ALLA;
            ...
            if(MedioArray[rand].visible){
                    ...
                    while(MedioArray[rand].yMedio <= ALTO+50){
                        al_rest(VELOCIDAD_ATAQUE);
                        MedioArray[rand].yMedio += MOV_ATAQUE;
                        if(MedioArray[rand].yMedio > FILA_NAVE -50){
                                if(rand%2 == 0)
                                        MedioArray[rand].xMedio += MOV_ATAQUE;
                                    else
                                        MedioArray[rand].xMedio-= MOV_ATAQUE;
                        }
                        else{
            ...
            if(MedioBajoArray[rand].visible){
                    ...
                    while(MedioBajoArray[rand].yMedio <= ALTO+50){
                        al_rest(VELOCIDAD_ATAQUE);
                        MedioBajoArray[rand].yMedio+=MOV_ATAQUE;
                        if(MedioBajoArray[rand].yMedio > FILA_NAVE -50){
                                if(rand%2 == 0)
                                        MedioBajoArray[rand].xMedio += MOV_ATAQUE;
                                    else
                                        MedioBajoArray[rand].xMedio-= MOV_ATAQUE;
                        }
                        else{
                ...
                if(BajoArray[rand].visible){
                        ...
                        while(BajoArray[rand].yBajo <= ALTO+20){
                            al_rest(VELOCIDAD_ATAQUE);
                            BajoArray[rand].yBajo+=MOV_ATAQUE;
                            // Verifica si hay colision de la nave con el marciano
                    ...
                    case 4:             //Para los ultimos marcianos
                {
                    ...
                    if(BajoBajoArray[rand].visible){
                            ...
                            while(BajoBajoArray[rand].yBajo <= ALTO+20){
                                al_rest(VELOCIDAD_ATAQUE);
                                BajoBajoArray[rand].yBajo+=MOV_ATAQUE;
                                if(BajoBajoArray[rand].yBajo > FILA_NAVE -50){
                                        if(rand%2 == 0)
                                                BajoBajoArray[rand].xBajo += MOV_ATAQUE;
                                            else
                                                BajoBajoArray[rand].xBajo-= MOV_ATAQUE;
                                }
                                else{
...
if (!al_install_keyboard()) {
    ...
    JuegoDatos->yBG = 0-ALTO;
    JuegoDatos->xBG = 0;
    JuegoDatos->jugando = 1;
    JuegoDatos->jugando = 1;
    JuegoDatos->fin_del_juego = 0;
    JuegoDatos->cantidad_vidas = 2;
    JuegoDatos->marciano_muerto = 0;
    JuegoDatos->tractor_evento = 0;
    redibujar = 1;


    //Declaración del tractor
    tractor = malloc(sizeof(tractor));
    tractor->tractor = al_load_bitmap("img/tractor.png");
    tractor->xTractor = MAS_ALLA;
    tractor->yTractor = MAS_ALLA;
    ...
    fuente = al_load_font("fnt/font.ttf",TAM_FUENTE,0);
...
// Cambio de fondo
    JuegoDatos->BG = al_load_bitmap("img/nube.png");
    al_set_window_title(Pantalla,"Galaga");
...
// Inicializa los marcianos y la nave
    ...
    // Hilo que actualiza el score
    hilo_score = al_create_thread(update_score,JuegoDatos);
    al_start_thread(hilo_score);

    // Hilo que espera el tractor beam
    hilo_tractor = al_create_thread(espera_tractor,JuegoDatos);
    //al_start_thread(hilo_tractor);
		...
		if(JuegoDatos->jugando){
                ...
                // Revisa si algun evento dice que terminó el juego
                    ...
                    if(redibujar && al_is_event_queue_empty(EventQueue)){
                            al_draw_bitmap(JuegoDatos->BG, JuegoDatos->xBG, JuegoDatos->yBG, 0);
                            //Texto del puntaje
                            al_draw_textf(fuente,COLOR_TEXTO, ANCHO-170,10,0,"Nivel: %d",Nivel);
                            al_draw_textf(fuente,COLOR_TEXTO, ANCHO-170,30,0,"Puntaje: %d",RespPuntaje);
                            al_draw_bitmap(tractor->tractor, tractor->xTractor, tractor->yTractor, 0);
                            dibujarNave(JuegoDatos);
